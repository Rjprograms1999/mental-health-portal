...
export const fetchAssignments = createAsyncThunk("assignments/fetchAll", async (_, thunkAPI) => {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/assignment`, {
...
export const assignUsers = createAsyncThunk("assignments/assignUsers", async ({ therapistId, assignedUsers }, thunkAPI) => {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/assignment/assign`, {
      method: "POST",
      headers: getAuthHeaders(),
...
export const getAssignedUsersForTherapist = createAsyncThunk("assignments/getAssignedUsersForTherapist", async (_, thunkAPI) => {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/assignment/my-users`, {
      headers: getAuthHeaders(),
    });
    if (!res.ok) {
      const error = await res.json();
...
export const removeUserFromTherapist = createAsyncThunk("assignments/removeUserFromTherapist", async ({ therapistId, userId }, thunkAPI) => {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/assignment/remove-user`, {
      method: "PUT",
      headers: getAuthHeaders(),
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(assignUsers.fulfilled, (state, action) => {
        ...
        const updatedTherapistId = typeof updated.therapistId === "object" ? updated.therapistId._id : updated.therapistId;

        const index = state.assignments.findIndex((a) => {
          const aTherapistId = typeof a.therapistId === "object" ? a.therapistId._id : a.therapistId;
      ...
      .addCase(removeUserFromTherapist.fulfilled, (state, action) => {
        ...
        const index = state.assignments.findIndex((a) => a.therapistId._id === updated.therapistId._id || a.therapistId === updated.therapistId);
