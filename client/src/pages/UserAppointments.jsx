...
import { getUserAppointments, updateAppointment } from "../redux/appointmentSlice";
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        {appointments.length === 0 ? (
          <div className="bg-blue-50 text-blue-700 rounded-lg p-4 text-center">You have no appointments.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="py-3 px-4 text-left text-gray-900 font-semibold">Therapist</th>
              ...
              <tbody>
                {appointments.map((appt) => (
                  <tr key={appt._id} className="border-b border-gray-200 hover:bg-gray-50">
                    <td className="py-3 px-4 text-gray-900">{appt.therapistId?.username || "Unknown"}</td>
                    ...
                    <td className="py-3 px-4">
                      {appt.status === "scheduled" || appt.status === "rescheduled" ? (
                        ...
                        <span className="text-gray-500 text-sm capitalize">{appt.status}</span>
                      )}
                    </td>
                  </tr>
                ))}
